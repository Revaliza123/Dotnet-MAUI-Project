...
public partial class DrinksPage : ContentPage
{
    ...
    public DrinksPage(ProductServices productService, DataSeedService seeder, CartService cartService)
    {
        InitializeComponent();
    ...
    private async Task LoadProducts()
    {
        try
        {
            ...
            if (products != null)
            {
                var drinkProducts = products.Where(p => p.Type == Product.ProductTypes.Drink).ToList();
                MenuCollection.ItemsSource = drinkProducts;
                EmptyLabel.IsVisible = !drinkProducts.Any();
            }
        }
        catch (Exception ex)
        {
    ...
    private async void OnCategoryClicked(object sender, EventArgs e)
    {
        if (sender is Button btn)
        {
            ...
            {
                await Navigation.PushAsync(new DessertPage(_productService, _seeder, _cartService));
            }
        }
    }
